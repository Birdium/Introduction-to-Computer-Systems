...
void SDL_BlitSurface(SDL_Surface *src, SDL_Rect *srcrect, SDL_Surface *dst, SDL_Rect *dstrect) {
  ...
  if (srcrect == NULL) {
    s_x = s_y = 0, w = src->w, h = src->h;
  }
  else {
    s_x = srcrect->x, s_y = srcrect->y, w = srcrect->w, h = srcrect->h;
  }
  if (dstrect == NULL){
    d_x = d_y = 0; 
    // w = dst->w < w ? dst->w : w; 
    ...
    d_x = dstrect->x;
    d_y = dstrect->y; 
    // w = dstrect->w < w ? dstrect->w : w; 
    // h = dstrect->h < h ? dstrect->h : h;
  }
  uint8_t *s_pix = src->pixels;
  uint8_t *d_pix = dst->pixels;
  int d_W = dst->w;
  int s_W = src->w;
  // printf("%d %d %d %d %d %d\n", s_x,s_y,d_x,d_y,w, h);
...
void SDL_FillRect(SDL_Surface *dst, SDL_Rect *dstrect, uint32_t color) {
  ...
  if (!dst->format->palette){
    ...
    if (color != palette->colors[col_ndx].val) printf("%x", color);
    for(int i = 0; i < h; i++) {
      for(int j = 0; j < w; j++) {
...
// static inline uint32_t from_8_to_32(SDL_Palette pal_color){
//   return 
// }
